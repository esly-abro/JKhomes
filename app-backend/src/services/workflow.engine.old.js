...
class WorkflowEngine {
   ...
   * Uses dynamic credentials from Settings model based on user
   */
  async executeWhatsApp(lead, config, context = {}) {
    try {
      ...
      // Get userId for dynamic credential lookup
      const userId = context.userId || lead.assignedTo || lead.assignedAgent;
      
      // Use Meta WhatsApp API service (handles credential lookup internally)
      const whatsappService = require('./whatsapp.service');
      
      if (config.template) {
        // Send template message with userId for dynamic credentials
        ...
        // Send text message with userId for dynamic credentials
   ...
   * Uses dynamic credentials from Organization model based on lead's assigned agent
   */
  async executeAICallWithResponse(lead, config, run) {
    try {
      ...
      // Determine userId for credential lookup
      ...
      // Make the call with dynamic credentials
      const result = await elevenLabsService.makeCall(lead.phone, {
        script: config?.script,
        voiceId: config?.voiceId,
        leadName: lead.name,
        leadData: lead,
        userId: userId, // Pass userId for dynamic credential lookup
  ...
  async executeAICall(lead, config, run = null) {
    try {
      ...
      // Determine userId for credential lookup
      // Priority: lead's assigned agent -> automation creator -> null (fallback to env)
      let userId = lead.assignedTo || lead.assignedAgent;
      if (!userId && run?.automation) {
        const automation = await Automation.findById(run.automation);
        userId = automation?.createdBy;
